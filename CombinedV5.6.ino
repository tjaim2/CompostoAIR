...
char keyLabel[15][20] = {" ", " ", " ", "SPIN ME", "Settings", "Set Timer", "Temperature Units", "Back", "-", "+", "Timer Units", "Back", "Back", "Back", "Back" };
...
uint8_t colorNitrogenStatus;
uint8_t colorPotassiumStatus;
uint8_t colorPhosphorousStatus;
uint8_t colorMoistureStatus;
uint8_t colorTemperatureStatus;
float nitrogenPercentage;
float phosphorousPercentage;
float potassiumPercentage;
int percentage;
float tempLow;
float tempHigh;
float tempActual;
...
#define us_from_mins 60000000  /* micro seconds to minutes*/
#define us_from_hours 3600000000    /*us to hours*/
#define ms_from_seconds 1000  /*milli seconds to seconds */
#define ms_from_mins 60000  /* milli seconds to minutes*/
#define ms_from_hours 3600000    /*ms to hours*/
...
struct stateDisplay {
  ...
  uint64_t unitsSleep[3]; //Used For Sleep : 0 = seconds, 1 = minutes, 2 = hours
  uint32_t colorStatus[4];  // Use for status indicators Green->Yellow->Red->Blue
...
RTC_DATA_ATTR uint64_t sleepScalar = 30;     /* ESP32 will go to sleep for this long. Specify units by multiplying by the desired "us_from_XXXX" when defining "time_in_us" below (Ex: if you want units to be seconds, multiply sleepScalar * us_from seconds) */
RTC_DATA_ATTR uint8_t sleepUnitSelect = 0;
uint64_t time_in_us = sleepScalar * display.unitsSleep[sleepUnitSelect];             /* use for esp_deep_sleep function */
...
unsigned long timeSensorRefresh = 10000;     /*Minimum Time between sensor refresh (in ms) */
RTC_DATA_ATTR unsigned long testTime;
RTC_DATA_ATTR unsigned long lastSpin;
...
stateSensors TMNPK = {0, 0 , 0x00, 0x00, 0x00}; //Temperature Moisture NP
//--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
/* Load Startup Values on Fresh boot */
  if (rebootCount == 0){
    lastSpin = millis();
    lastUpdate = 3000000;
    autoSpinScalar = 50;
    autoSpinUnitSelect = 0;
    //sleepUnitSelect = 0;
    //sleepScalar = 30;
...
//    tft.fillRect(3, 3, 204, 76, display.colorStatus[3]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[3]); //tft.setTextColor(display.colorStatus[3], TFT_BLACK);
//  } else if (tempActual >= tempHigh) {  // Adjusted condition for "High"
//    tft.fillRect(3, 3, 204, 76, display.colorStatus[2]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[2]);//tft.setTextColor(display.colorStatus[2], TFT_BLACK);
//  } else {
//    tft.fillRect(3, 3, 204, 76, display.colorStatus[0]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[0]);//tft.setTextColor(display.colorStatus[0], TFT_BLACK);
//  }
  tft.setTextColor(colorText1 ,display.colorStatus[colorTemperatureStatus]); //tft.setTextColor(display.colorStatus[0], TFT_BLACK);
  tft.fillRect(3, 3, 204, 76, display.colorStatus[colorTemperatureStatus]);
...
//    tft.fillRect(3, 83, 68, 76, display.colorStatus[2]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[2]); //tft.setTextColor(display.colorStatus[2], TFT_BLACK);
//  } else if (nitrogenPercentage <= (2.0 - 0.5)) {
//    tft.fillRect(3, 83, 68, 76, display.colorStatus[3]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[3]); //tft.setTextColor(display.colorStatus[1], TFT_BLACK);
//  } else {
//    tft.fillRect(3, 83, 68, 76, display.colorStatus[0]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[0]); //tft.setTextColor(display.colorStatus[0], TFT_BLACK);
//  }
  //tft.print(nitrogenPercentage);
  tft.setTextColor(colorText1 ,display.colorStatus[colorNitrogenStatus]); //tft.setTextColor(display.colorStatus[0], TFT_BLACK);
  tft.fillRect(3, 83, 68, 76, display.colorStatus[colorNitrogenStatus]);
  tft.println("N");

/* Phosphorous */
tft.setCursor(103, topEdge2, 1);
//if (phosphorousPercentage >= 1.0 + 0.25) {
//    tft.fillRect(72, 83, 68, 76, display.colorStatus[2]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[2]); //tft.setTextColor(display.colorStatus[2], TFT_BLACK);
//} else if (phosphorousPercentage <= 0.5 - 0.25) {
//    tft.fillRect(72, 83, 68, 76, display.colorStatus[3]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[3]); //tft.setTextColor(display.colorStatus[2], TFT_BLACK);
//} else {
//    tft.fillRect(72, 83, 68, 76, display.colorStatus[0]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[0]); //tft.setTextColor(display.colorStatus[0], TFT_BLACK);
//}
  tft.setTextColor(colorText1 ,display.colorStatus[colorPhosphorousStatus]); //tft.setTextColor(display.colorStatus[0], TFT_BLACK);
  tft.fillRect(72, 83, 68, 76, display.colorStatus[colorPhosphorousStatus]);
  tft.println("P");
...
//    tft.fillRect(141, 83, 68, 76, display.colorStatus[2]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[2]); //tft.setTextColor(display.colorStatus[2], TFT_BLACK);
//} else if (potassiumPercentage <= 2.0 - 0.5) {
//    tft.fillRect(141, 83, 68, 76, display.colorStatus[3]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[3]); //tft.setTextColor(display.colorStatus[2], TFT_BLACK);
//} else {
//    tft.fillRect(141, 83, 68, 76, display.colorStatus[0]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[0]); //tft.setTextColor(display.colorStatus[0], TFT_BLACK);
//}
  tft.setTextColor(colorText1 ,display.colorStatus[colorPotassiumStatus]); //tft.setTextColor(display.colorStatus[0], TFT_BLACK);
  tft.fillRect(141, 83, 68, 76, display.colorStatus[colorPotassiumStatus]);
  tft.println("K");

/* Display Moisture Sensor */
  tft.setCursor(65, topEdge3, 1);
//  if (percentage <= (dryThreshold - 5)) {  // Adjusted condition for "Low"
//    tft.fillRect(3, 163, 204, 76, display.colorStatus[3]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[3]); //tft.setTextColor(display.colorStatus[2], TFT_BLACK);
//  } else if (percentage >= (wetThreshold + 5)) {  // Adjusted condition for "High"
//    tft.fillRect(3, 163, 204, 76, display.colorStatus[2]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[2]); //tft.setTextColor(display.colorStatus[1], TFT_BLACK);
//  } else {
//    tft.fillRect(3, 163, 204, 76, display.colorStatus[0]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[0]); //tft.setTextColor(display.colorStatus[0], TFT_BLACK);
//  }
  tft.fillRect(3, 163, 204, 76, display.colorStatus[colorMoistureStatus]);
  delay(1);
  tft.setTextColor(colorText1 ,display.colorStatus[colorMoistureStatus]); //tft.setTextColor(display.colorStatus[2], TFT_BLACK);
...
void updateSensors(){
  if (((millis() - lastUpdate) > timeSensorRefresh) || (rebootCount == 0)){
    ...
    nitrogenPercentage = (TMNPK.nitrogen / maxVal) * 100;
    phosphorousPercentage = (TMNPK.phosphorous / maxVal) * 100;
    potassiumPercentage = (TMNPK.potassium / maxVal) * 100;
    percentage = map(TMNPK.moisture, dry, wet, 0, 100);
/* Update Temperature Status */    
    if(display.unitsTemp == 0){
      tempLow = temperatureCMin - 5;
      tempHigh = temperatureCMin + 5;
      tempActual = TMNPK.temperatureC;
    } 
    
    else if (display.unitsTemp == 1){
      tempLow = (temperatureCMin - 5) * (9 / 5) + 32;
      tempHigh = (temperatureCMin + 5) * (9 / 5) + 32;
      tempActual = TMNPK.temperatureC * (9 / 5) + 32;
    }
  if (tempActual <= tempLow) {  // Adjusted condition for "Low"
    colorTemperatureStatus = 3;
//    tft.fillRect(3, 3, 204, 76, display.colorStatus[3]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[3]); //tft.setTextColor(display.colorStatus[3], TFT_BLACK);
  } else if (tempActual >= tempHigh) {  // Adjusted condition for "High"
    colorTemperatureStatus = 2;
//    tft.fillRect(3, 3, 204, 76, display.colorStatus[2]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[2]);//tft.setTextColor(display.colorStatus[2], TFT_BLACK);
  } else {
    colorTemperatureStatus = 0;
//    tft.fillRect(3, 3, 204, 76, display.colorStatus[0]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[0]);//tft.setTextColor(display.colorStatus[0], TFT_BLACK);
  }
...
/* Update Nitrogen Status */
    if (nitrogenPercentage >= (2.0 + 0.5)) {
      colorNitrogenStatus = 2;
  } else if (nitrogenPercentage <= (2.0 - 0.5)) {
      colorNitrogenStatus = 3;
  } else {
    colorNitrogenStatus = 0;
  }

    /* Update Phosphorous Status */
    if (phosphorousPercentage >= 1.0 + 0.25) {
      colorPhosphorousStatus = 2;
  } else if (phosphorousPercentage <= 0.5 - 0.25) {
      colorPhosphorousStatus = 3;
  } else {
      colorPhosphorousStatus = 0;
  }

    /* Update Potassium Status */
    if (potassiumPercentage >= 2.0 + 0.5) {
      colorPotassiumStatus = 2;
  } else if (potassiumPercentage <= 2.0 - 0.5) {
      colorPotassiumStatus = 3;
  } else {
      colorPotassiumStatus = 0;
  }
 }

/* Update Moisture Status */
  if (percentage <= (dryThreshold - 5)) {  // Adjusted condition for "Low"
    colorMoistureStatus = 3;
//    tft.fillRect(3, 163, 204, 76, display.colorStatus[3]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[3]); //tft.setTextColor(display.colorStatus[2], TFT_BLACK);
  } else if (percentage >= (wetThreshold + 5)) {  // Adjusted condition for "High"
    colorMoistureStatus = 2;
//    tft.fillRect(3, 163, 204, 76, display.colorStatus[2]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[2]); //tft.setTextColor(display.colorStatus[1], TFT_BLACK);
  } else {
    colorMoistureStatus = 0;
//    tft.fillRect(3, 163, 204, 76, display.colorStatus[0]);
//    delay(1);
//    tft.setTextColor(colorText1 ,display.colorStatus[0]); //tft.setTextColor(display.colorStatus[0], TFT_BLACK);
  }
...
/* Main Menu*/
  ...
  case 2: if (display.newPage){
          ...
          tft.setTextSize(3);
          tft.setCursor(20, 26, 1);
          tft.setTextColor(colorText1, display.colorStatus[colorNitrogenStatus]);
          tft.print(nitrogenPercentage);
          tft.println("%:");
          
          tft.setCursor(20, 74, 1);
          tft.setTextColor(colorText1, display.colorStatus[colorPhosphorousStatus]);
          tft.print(phosphorousPercentage);
          tft.println("%:");
                    
          tft.setCursor(20, 122, 1);
          tft.setTextColor(colorText1, display.colorStatus[colorPotassiumStatus]);
  ...
  case 5: if (display.newPage){
          ...
          tft.println(timerString[autoSpinUnitSelect]);
          break;
...
switch(display.page)
  ...
  /* NPK Screen */
  case 2:   tft.fillScreen(colorBackground);
            tft.drawRect(17, 17, 288, 48, colorBorder);
            tft.fillRect(18, 18, 286, 46, display.colorStatus[colorNitrogenStatus]);
            tft.drawRect(17, 65, 288, 48, colorBorder);
            tft.fillRect(18, 66, 286, 46, display.colorStatus[colorPhosphorousStatus]);
            tft.drawRect(17, 113, 288, 48, colorBorder);
            tft.fillRect(18, 114, 286, 46, display.colorStatus[colorPotassiumStatus]);
            
            key[13].initButtonUL(&tft, 17, 176, 288, 48 , colorBorder, colorBackground, keyColor[13], keyLabel[13], 1);
            tft.drawRect(17, 176, 288, 48, colorBorder);
            tft.fillRect(18, 177, 286, 46, colorBackground);
            tft.setTextSize(2);
            tft.setCursor(140, 185, 1);
...
/* Timer Settings Screen */
   case 5:  tft.setTextSize(3);//tft.setTextSize(2);
            ...
            tft.fillRect(12, 33, 42, 16, TFT_DARKGREY);
            ...
            tft.fillRect(268, 33, 42, 16, TFT_DARKGREY);
...
/* Main Menu */
  ...
  /* NPK Screen */
  case 2:  
          if (display.pressed && key[13].contains(touchX, touchY)) {
            key[13].press(true);  // tell the button it is pressed
          } else {
            key[13].press(false);  // tell the button it is NOT pressed
          }
          
          
           // Check if any key has changed state        
            if (key[13].justReleased()){      
                  lastInput = millis();
                  display.page = 0;
                  display.newPage = true;
              }
            ...
            if (key[b].justReleased()){
                ...
                if (b == 8) {
                  ...
                  autoSpinScalar = (autoSpinScalar + 1) % 100;
                   ...
                   lastInput = millis();
                   autoSpinUnitSelect = (autoSpinUnitSelect + 1) % 3;
                }
                else if (b == 11){
                  lastInput = millis();
                  display.page = 0;
