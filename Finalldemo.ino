...
void displayData(float temperature, int sensorValue, int percentage, byte val1, byte val2, byte val3);
void spinIt();
void yonDelay();
...
#define sleep_time  1000      /* ESP32 will go to sleep for this long. Specify units by multiplying by the desired "us_from_XXXX" when defining "time_in_us" below (Ex: if you want units to be seconds, multiply sleep_time * us_from seconds) */
unsigned long time_in_us = sleep_time * us_from_seconds;  /* use for esp_deep_sleep function */
unsigned long timeUntilSleep = 100000000; /* How long without an input until system goes to sleep (in ms) */
unsigned long autoSpinTime =   100000000; /* How long between automatic turns (in ms) */
...
void loop() {
  ...
  if (temperatureC != DEVICE_DISCONNECTED_C && sensorValue >= moistureMin && sensorValue <= moistureMax) {
    displayData(temperatureC, sensorValue, percentage, val1, val2, val3);
  } else {
